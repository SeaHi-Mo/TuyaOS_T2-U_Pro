author: Tuya
summary: wifi qr activate demo
id: tuyaos_demo_wifi_qr_activate
categories: wifi_qr_activate,markdown
environments: Web
status: Published

# tuyaos_demo_wifi_qr_activate

## 简介

`tuyaos_demo_wifi_qr_activate`  示例的目的是在体现 TuyaOS 单品开发的基本能力下，提供一个尽可能简单的示例。方便开发者借助此示例熟悉 TuyaOS 开发流程，验证手上的模组通过扫码直连wifi热点进行设备的激活，典型的应用有可视挖耳勺、儿童相机等。
...
例：如果我们编译的是 apps 文件夹中的 `tuyaos_demo_wifi_qr_activate` 工程，编译版本为1.0.0，那么生成的固件将会放在 `apps/tuyaos_demo_wifi_qr_activate/output/1.0.0`中。

| 文件名                                       | 功能                                   |
| -------------------------------------------- | -------------------------------------- |
| `tuyaos_demo_wifi_qr_activate_QIO_1.0.0.bin` | 生产区固件：boot+用户区固件+检测固件。 |
| `tuyaos_demo_wifi_qr_activate_UA_1.0.0.bin`  | 用户区固件                             |
| `tuyaos_demo_wifi_qr_activate_UG_1.0.0.bin`  | 升级固件                               |
...
在开发过程遇到问题，您可以登录 [TuyaOS 开发者论坛](https://www.tuyaos.com/viewforum.php?f=2) 发帖进行沟通咨询。

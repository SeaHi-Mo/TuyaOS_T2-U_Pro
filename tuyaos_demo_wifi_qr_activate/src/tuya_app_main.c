...
#define UUID        "bf25dac0b2b2f6a1"
#define AUTHKEY     "DRiaFdK6VMp6Q6AM8HCjaVFL8JifkhBO"
#define PIN         "88578925"
#define APSSID      "SmartLife-6057"
#define SHORTURL    "https://m.smart321.com/AYgFFdPb"
...
STATIC DELAYED_WORK_HANDLE wq_print_qr;
STATIC VOID __soc_dev_print_qr_work(VOID *data)
{
    TAL_PR_DEBUG_RAW("*************************************************************\r\n");
    TAL_PR_DEBUG_RAW("short url : %s\r\n", SHORTURL);
    TAL_PR_DEBUG_RAW("*************************************************************\r\n");

    if (get_gw_active() >= ACTIVATED) {
        TAL_PR_DEBUG("device has been activated");
        return;
    }
    example_qrcode_string(SHORTURL, user_log_output_cb, 0);
    return;
}

STATIC OPERATE_RET __soc_dev_run_evt(VOID *data)
{
    OPERATE_RET rt = OPRT_OK;
    TAL_PR_DEBUG("SOC Rev Run Event");
    rt = tal_workq_init_delayed(WORKQ_SYSTEM, __soc_dev_print_qr_work, NULL, &wq_print_qr);
    if (rt != OPRT_OK) {
        TAL_PR_ERR("tal_workq_init_delayed error");
        return rt;
    }
    rt = tal_workq_start_delayed(wq_print_qr, 5 * 1000, LOOP_ONCE);
    if (OPRT_OK != rt) {
        TAL_PR_ERR("start tm err:%d", rt);
    }
    return rt;
}
...
OPERATE_RET __soc_device_init(VOID_T)
{
    ...
    ty_subscribe_event(EVENT_INIT, "quickstart", __soc_dev_run_evt, SUBSCRIBE_TYPE_NORMAL);
    ...
    WF_GW_PROD_INFO_S prod_info = {UUID, AUTHKEY, APSSID, NULL, PIN};

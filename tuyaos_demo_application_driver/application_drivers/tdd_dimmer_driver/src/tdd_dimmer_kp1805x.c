...
STATIC OPERATE_RET __tdd_dimmer_kp1805x_output(IN DIMMER_DRV_HANDLE drv_handle, LIGHT_RGBCW_U *p_rgbcw)
{
    ...
    //每次发送数据前给驱动启停信号，防止误识别
    tdd_sw_i2c_start(p_drv->i2c_pin);
    tdd_sw_i2c_stop(p_drv->i2c_pin);
    tdd_sw_i2c_clk_lower_pulse(p_drv->i2c_pin);//根据驱动协议增加scl下脉冲信号

    i2c_flags = I2C_FLAG_WR | I2C_FLAG_NO_ADDR | I2C_FLAG_IGNORE_NACK;

    rt = tdd_dimmer_i2c_flag_send_data(p_drv->i2c_pin, i2c_flags, (UCHAR_T *)p_prot, SIZEOF(IIC_PROT_T));

    if(OPRT_INVALID_PARM != rt){
        tdd_sw_i2c_clk_lower_pulse(p_drv->i2c_pin);
    }
 ...
 * @return OPRT_OK on success. Others on error, please refer to "tuya_error_code.h"
 */
STATIC OPERATE_RET __tdd_dimmer_kp1805x_config(IN DIMMER_DRV_HANDLE drv_handle, DIMMER_DRV_CMD_E cmd, VOID_T *arg)
{
    DIMMER_KP1805X_DRV_T *p_drv = (DIMMER_KP1805X_DRV_T *)drv_handle;
                                                 ...
                                                 DIMMER_KP1805X_CFG_T *p_chip_cfg, KP1805X_TYPE_E tp)
{
    ...
    drv_info.chip_tp    = (KP18059_TYPE == tp) ? DIMMER_CHIP_KP18059ESPA : DIMMER_CHIP_KP18058ESPA;
    drv_info.resolution = KP1805X_GRAY_VAL_MAX; 

    intfs.open   = __tdd_dimmer_kp1805x_open;
    intfs.close  = __tdd_dimmer_kp1805x_close;
    intfs.output = __tdd_dimmer_kp1805x_output;
    intfs.config = __tdd_dimmer_kp1805x_config;

 ...
 * @brief thread
 */
STATIC THREAD_HANDLE sg_sensor_thrd = NULL;
STATIC SEM_HANDLE sg_task_sem;
                                         ...
                                         IN SR_ELE_CFG_T *ele_cfg, IN SR_RSRC_T *resource)
{
    ...
    device = (SR_DEV_NODE_T *)tal_malloc(SIZEOF(SR_DEV_NODE_T));
    if (NULL == device) {
        TAL_PR_ERR("Malloc for %s failed !", name);
        return OPRT_MALLOC_FAILED;
    }
    memset(device, 0, SIZEOF(SR_DEV_NODE_T));

    // save the name, resource and interface
    strncpy(device->name, name, SR_DEV_NAME_MAX_LEN);
    memcpy(&device->resource, resource, SIZEOF(SR_RSRC_T));
    device->intfs = intfs;
...
STATIC OPERATE_RET __tdl_sensor_load_config(IN SR_DEV_NODE_T *device, IN SR_DEV_CFG_T *config)
{
    ...
    if (NULL != device->ele_sub) {
        if (NULL != device->ele_sub->rule) {
            tal_free(device->ele_sub->rule);
            device->ele_sub->rule = NULL;
        }
        tal_free(device->ele_sub);
        device->ele_sub = NULL;
    }

    device->ele_sub = (SR_ELE_SUB_CFG_T *)tal_malloc(SIZEOF(SR_ELE_SUB_CFG_T));
    if (NULL == device->ele_sub) {
        TAL_PR_ERR("Malloc for ele_sub failed !");
        return OPRT_MALLOC_FAILED;
    }
    memset(device->ele_sub, 0, SIZEOF(SR_ELE_SUB_CFG_T));

    UINT_T rule_size = SIZEOF(SR_ELE_SUB_RULE_T) * config->ele_sub->num;
    device->ele_sub->rule = (SR_ELE_SUB_RULE_T *)tal_malloc(rule_size);
    if (NULL == device->ele_sub->rule) {
    ...
    memcpy(device->ele_sub->rule, config->ele_sub->rule, rule_size);
...
STATIC VOID_T __tdl_sensor_soft_timer_cb(IN TIMER_ID timer_id, IN VOID_T *arg)
{
    ...
    tal_semaphore_post(sg_task_sem);
    return;
}
...
STATIC OPERATE_RET __tdl_sensor_hard_timer_init(VOID_T)
{
    ...
    for (i = 0; i < TUYA_TIMER_NUM_MAX; i++) {
...
STATIC OPERATE_RET __tdl_sensor_prepare_app_fifo(IN SR_DEV_NODE_T *device)
{
    ...
    if (SR_DATA_TP_ELE == __tdl_sensor_get_data_tp(device->ele_num)) {
        ...
        if (NULL != device->app_fifo.ele) {
            tal_free(device->app_fifo.ele);
            device->app_fifo.ele = NULL;
        }
        total_size = (SIZEOF(SR_ELE_BUFF_T) + SIZEOF(SR_VAL_U) * num) * ele_num;
        device->app_fifo.ele = (SR_ELE_BUFF_T *)tal_malloc(total_size);
        if (NULL == device->app_fifo.ele) {
            return OPRT_MALLOC_FAILED;
        }
        memset(device->app_fifo.ele, 0, total_size);
        ...
        if (NULL != device->app_fifo.raw.data) {
            tal_free(device->app_fifo.raw.data);
            device->app_fifo.raw.data = NULL;
        }
        total_size = SIZEOF(SR_RAW_DATA_T) * num;
        device->app_fifo.raw.data = (SR_RAW_DATA_T *)tal_malloc(total_size);
        if (NULL == device->app_fifo.raw.data) {
            return OPRT_MALLOC_FAILED;
        }
        memset(device->app_fifo.raw.data, 0, total_size);
    }
...
STATIC OPERATE_RET __tdl_sensor_dev_close(INOUT SR_DEV_NODE_T *device)
{
    ...
    memset(&device->work_param, 0, SIZEOF(SR_WORK_PARAM_T));
...
STATIC OPERATE_RET __tdl_sensor_load_ele_value(INOUT SR_DEV_NODE_T *device)
{
    ...
    // prepare buffer
    ele_data = (SR_ELE_DATA_T *)tal_malloc(SIZEOF(SR_ELE_DATA_T) * ele_num);
    if (NULL == ele_data) {
        op_ret = OPRT_MALLOC_FAILED;
        goto LOAD_ELE_END;
    }
    memset(ele_data, 0, SIZEOF(SR_ELE_DATA_T)*ele_num);
    for (i = 0; i < ele_num; i++) {
    ...
    if (SR_ELE_SUB_TP_SINGLE == device->ele_sub->tp) {
        is_need_filter = tal_malloc(ele_num);
        if (NULL == is_need_filter) {
            op_ret = OPRT_MALLOC_FAILED;
            goto LOAD_ELE_END;
        }
        memset(is_need_filter, 0, ele_num);
    }
    ...
    for (i = 0; i < ele_num; i++) {
        if (NULL == device->ele_sub || SR_ELE_SUB_TP_GROUP == device->ele_sub->tp ||
            ...
            memcpy(&device->app_fifo.ele[i].val[val_num], &ele_data[i].val, SIZEOF(SR_VAL_U));
...
STATIC VOID_T __tdl_sensor_inform_ele_value(INOUT SR_DEV_NODE_T *device)
{
    ...
    if (NULL == device->ele_sub ||
        ...
        if (device->app_fifo.ele[0].val_num >= device->fifo_size) {
            ...
            for (i = 0; i < ele_num; i++) {
                memset(device->app_fifo.ele[i].val, 0, SIZEOF(SR_VAL_U)*device->fifo_size);
        ...
        for (i = 0; i < ele_num; i++) {
            ...
            memset(device->app_fifo.ele[i].val, 0, SIZEOF(SR_VAL_U)*device->app_fifo.ele[i].val_num);
            device->app_fifo.ele[i].val_num = 0;
        }
    }
...
STATIC VOID_T __tdl_sensor_load_and_inform_raw_value(INOUT SR_DEV_NODE_T *device)
{
    ...
    if (device->app_fifo.raw.len >= device->fifo_size) {
        ...
        memset(device->app_fifo.raw.data, 0, SIZEOF(SR_RAW_DATA_T)*device->app_fifo.raw.len);
        device->app_fifo.raw.len = 0;
    }
...
STATIC VOID_T __tdl_sensor_main_task(PVOID_T args)
{
    ...
    while (1) {
        tal_semaphore_wait(sg_task_sem, SEM_WAIT_FOREVER);
    ...
    tal_thread_delete(sg_sensor_thrd);
    sg_sensor_thrd = NULL;
}
...
STATIC OPERATE_RET __tdl_sensor_hub_frame_init(VOID_T)
{
    ...
    op_ret = tal_thread_create_and_start(&sg_sensor_thrd, NULL, NULL, __tdl_sensor_main_task, NULL, &thrd_cfg);
    if (op_ret !=  OPRT_OK) {
        return op_ret;
    }

    // create a semaphore for tdl_sensor_hub
    op_ret = tal_semaphore_create_init(&sg_task_sem, 0, 1);
    if (op_ret !=  OPRT_OK) {
        return op_ret;
    }

 ...
 * @return none
 */
STATIC VOID __tdl_get_cct_warm(IN LIGHT_RGBCW_T *p_src_rgbcw, IN LIGHT_RGBCW_T *p_target_rgbcw,\
                               INOUT LIGHT_RGBCW_T *p_curr_rgbcw)
{
    //cct驱动渐变开关需要特殊处理w值变化
...
STATIC OPERATE_RET __tdl_dimmer_output(IN DIMMER_NODE_T *device, IN LIGHT_RGBCW_U *p_rgbcw)
{
    ...
    UINT_T i = 0;
    USHORT_T drv_resolution = 0;

    drv_resolution = device->drv_info.resolution;

    for(i = 0; i < LIGHT_COLOR_CHANNEL_MAX; i++){
        device->output_rgbcw.array[i] = (p_rgbcw->array[i] >= device->app_resolution) ? drv_resolution : \
                                        (p_rgbcw->array[i] * drv_resolution / device->app_resolution);
    }
 ...
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
 */
STATIC OPERATE_RET __tdl_dimmer_register(IN CHAR_T *name, IN DIMMER_DRV_INTFS_T *p_intfs, \
                                        DIMMER_DRV_INFO_T *p_info, IN DIMMER_DRV_HANDLE drv_handle)
{
    ...
    memcpy(&device->drv_info, p_info,  SIZEOF(DIMMER_DRV_INFO_T));
                                ...
                                IN DIMMER_DRV_HANDLE drv_handle)
{
    if ((NULL == name || NULL == p_intfs  || NULL == drv_handle) ||
        (NULL == p_intfs->open || NULL == p_intfs->close || NULL == p_intfs->output || NULL ==  p_intfs->config)) {
        return OPRT_INVALID_PARM;
    }

    return __tdl_dimmer_register(name, p_intfs, p_info, drv_handle);
}
...
OPERATE_RET tdl_dimmer_output(IN TDL_DIMMER_HANDLE handle, IN LIGHT_RGBCW_T *p_rgbcw)
{
    ...
    if(DIMMER_DRIVER_PWM_CCT == device->drv_info.drv_tp){
        __tdl_get_cct_warm(&last_rgbcw, p_rgbcw, &device->rgbcw.s);
    }
...
OPERATE_RET tdl_dimmer_config(IN TDL_DIMMER_HANDLE handle, TDL_DIMMER_CMD_E cmd, INOUT VOID *arg)
{
    ...
    switch(cmd) {
    case DIMMER_CMD_GET_DRV_TYPE:{
        DIMMER_DRIVER_TYPE_E *p_type = NULL;
        if(NULL == arg) {
            rt = OPRT_INVALID_PARM;
            break;
        }
        p_type = (DIMMER_DRIVER_TYPE_E *)arg;
        *p_type = device->drv_info.drv_tp;
    }
    break;
    case DIMMER_CMD_GET_DRV_COLOR_CH:{
        LIGHT_DRV_COLOR_CH_E *p_color_ch = NULL;
        if(NULL == arg) {
            rt = OPRT_INVALID_PARM;
            break;
        }
        p_color_ch = (LIGHT_DRV_COLOR_CH_E *)arg;
        *p_color_ch = device->drv_info.color_ch;
    }
    break;
    case DIMMER_CMD_GET_CHIP_TYPE:{
        DIMMER_CHIP_TYPE_E *p_chip_type = (DIMMER_CHIP_TYPE_E *)arg;
        if(NULL == arg) {
            rt = OPRT_INVALID_PARM;
            break;
        }
        *p_chip_type = device->drv_info.chip_tp;
    }
    break;
    case DIMMER_CMD_GET_HARDWARE_CFG:{
        rt = device->intfs.config(device->drv_handle, DRV_CMD_GET_HARDWARE_CFG, arg);
    }
    break;
    default:break;
    }

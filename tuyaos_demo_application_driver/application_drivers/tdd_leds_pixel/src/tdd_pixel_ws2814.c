 ...
 */
OPERATE_RET tdd_ws2814_driver_open(OUT DRIVER_HANDLE_T *handle, IN unsigned short pixel_num)
{
 ...
 * @return: success -> 0  fail -> else
 */
OPERATE_RET tdd_ws2814_driver_send_data(IN DRIVER_HANDLE_T handle, IN unsigned short *data_buf, IN unsigned int buf_len)
{
 ...
 * @return: success -> 0  fail -> else
 */
OPERATE_RET tdd_ws2814_driver_close(IN DRIVER_HANDLE_T *handle)
{
    OPERATE_RET ret = OPRT_OK;
    DRV_PIXEL_TX_CTRL_T *tx_ctrl = NULL;
 ...
 */
OPERATE_RET tdd_ws2814_driver_register(IN CHAR_T *driver_name, IN PIXEL_DRIVER_CONFIG_T *init_param)
{
    OPERATE_RET ret = OPRT_OK;

...
/**
 * @function: tdd_sk6812_driver_open
 * @brief: 打开（初始化）设备
 ...
 */
OPERATE_RET tdd_sk6812_driver_open(OUT DRIVER_HANDLE_T *handle, IN unsigned short pixel_num)
{
...
/**
 * @function: tdd_sk6812_driver_send_data
 ...
 * @return: success -> 0  fail -> else
 */
OPERATE_RET tdd_sk6812_driver_send_data(IN DRIVER_HANDLE_T handle, IN unsigned short *data_buf, IN unsigned int buf_len)
{
...
/**
 * @function: tdd_sk6812_driver_close
 * @brief: 关闭设备（资源释放）
 * @param[in]: *handle -> 设备句柄
 * @return: success -> 0  fail -> else
 */
OPERATE_RET tdd_sk6812_driver_close(IN DRIVER_HANDLE_T *handle)
{
    OPERATE_RET ret = OPRT_OK;
    DRV_PIXEL_TX_CTRL_T *tx_ctrl = NULL;
 ...
 */
OPERATE_RET tdd_sk6812_driver_register(IN CHAR_T *driver_name, IN PIXEL_DRIVER_CONFIG_T *init_param)
{
    OPERATE_RET ret = OPRT_OK;
    PIXEL_DRIVER_INTFS_T intfs;
    PIXEL_ATTR_T arrt = {0};

    intfs.open = tdd_sk6812_driver_open;
    intfs.output = tdd_sk6812_driver_send_data;
    intfs.close = tdd_sk6812_driver_close;
